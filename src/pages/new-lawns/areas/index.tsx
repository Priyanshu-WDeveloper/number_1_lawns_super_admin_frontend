...
import { useGetNLAreasQuery, useDeleteNLAreaMutation, useUpdateNLAreaMutation } from '@/API/new-lawns-api';
...
export default function AreasListPage() {
  ...
  const { data, isLoading } = useGetNLAreasQuery({
    page,
    limit,
  ...
  const [deleteArea] = useDeleteNLAreaMutation();
  const [updateArea] = useUpdateNLAreaMutation();

  const handleToggleStatus = async (area: NewLawnArea) => {
    try {
      await updateArea({ id: area._id, status: area.status === 'active' ? 'inactive' : 'active' }).unwrap();
      toast.success(`Area ${area.status === 'active' ? 'deactivated' : 'activated'} successfully`);
    } catch (err) {
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await deleteArea(deleteId).unwrap();
      toast.success('Area deleted successfully');
      setDeleteId(null);
    } catch (err) {
      toast.error(getErrorMessage(err, 'Failed to delete area'));
    }
  };

  const columns: ColumnDef<NewLawnArea>[] = [
    {
      accessorKey: 'name',
      header: 'Name',
    },
    ...
    },
    {
      ...
      header: 'Description',
      cell: (row) => (
        <span className="max-w-[300px] truncate block">{row.description}</span>
      ),
    },
    {
      accessorKey: 'status',
      ...
      cell: (row) => (
        <div className="flex items-center gap-1">
          <ActionButton
            ...
            intent="edit"
            onClick={() => navigate(NEW_LAWNS_ROUTES.AREAS_EDIT.replace(':id', row._id), { state: { area: row } })}
          />
          <ActionButton
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={() => navigate(NEW_LAWNS_ROUTES.AREAS_EDIT.replace(':id', row._id), { state: { area: row } })}>
                Edit
              </DropdownMenuItem>
  ...
  return (
    <SuperAdminLayout>
      <Navbar title="New Lawns Areas" subtitle="Manage service areas" superAccess />
      <div className="flex-1 px-6 pb-6">
        <DataTable<NewLawnArea>
          data={data?.items ?? []}
          columns={columns}
          title="Areas"
          description="Manage service areas"
          addButtonLabel="Add Area"
          onAddClick={() => navigate(NEW_LAWNS_ROUTES.AREAS_CREATE)}
