...
import { useUpdateNLFAQMutation } from '@/API/new-lawns-api';
...
export default function EditFAQPage() {
  ...
  const passedData = location.state?.faq as NewLawnFAQ | undefined;

  const [updateFAQ, { isLoading }] = useUpdateNLFAQMutation();
  ...
  useEffect(() => {
    if (!passedData) {
      toast.error('No FAQ data provided');
  ...
  const onSubmit = async (data: FAQFormData) => {
    try {
      await updateFAQ({ id: passedData._id, ...data }).unwrap();
  ...
  return (
    <SuperAdminLayout>
      <Navbar title="Edit FAQ" subtitle="Update frequently asked question" superAccess />
