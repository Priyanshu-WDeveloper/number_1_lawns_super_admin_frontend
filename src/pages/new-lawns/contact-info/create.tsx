...
import { useCreateNLContactInfoMutation } from '@/API/new-lawns-api';
...
export default function CreateContactInfoPage() {
  ...
  const [createItem, { isLoading }] = useCreateNLContactInfoMutation();
  const form = useForm<FormData>({
    resolver: zodResolver(schema) as any,
    defaultValues: { type: 'phone', label: '', value: '', icon: '', order: 0, status: 'active' },
  });
    ...
    <SuperAdminLayout>
      <div className="flex h-full flex-col">
        <div className="flex-1 w-full overflow-y-auto p-5 md:pl-10">
          ...
          <Navbar title="Add Contact Info" subtitle="Add a new contact information entry" showWelcome={false} superAccess />
            ...
            <div className="rounded-xl border border-border bg-white p-6 shadow-sm space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-sm font-medium">Type <span className="text-primary">*</span></label>
                  <Select onValueChange={(v) => form.setValue('type', v as any)} defaultValue="phone">
                ...
                <div className="space-y-2">
                  ...
                  {form.formState.errors.order && <p className="text-sm text-red-500">{form.formState.errors.order.message}</p>}
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">Label <span className="text-primary">*</span></label>
                <Input {...form.register('label')} placeholder="e.g. Main Office, Support Email" className="h-12 rounded-xl border-border bg-background" />
              ...
              <div className="space-y-2">
                ...
                <Input {...form.register('value')} placeholder="e.g. +64 22 323 4429" className="h-12 rounded-xl border-border bg-background" />
                {form.formState.errors.value && <p className="text-sm text-red-500">{form.formState.errors.value.message}</p>}
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">Icon</label>
                <Input {...form.register('icon')} placeholder="Icon name or URL" className="h-12 rounded-xl border-border bg-background" />
              </div>
              <div className="space-y-2">
